...
# ---------------------------------------------------------------------------
# GET /api/v1/quantum/circuit/ascii — Return authentic Qiskit circuit gallery & QASM
# ---------------------------------------------------------------------------
@app.get("/api/v1/quantum/circuit/ascii")
async def get_quantum_circuit_ascii(circuit_type: str = "zz_linear", qubits: int = 8):
    circuits_data = {
        "zz_linear": {
            "id": "zz_linear",
...
q_7: ┤ Ry(θ[7]) ├┤ X ├┤ Ry(θ[15])├┤ X ├┤ Ry(θ[23])├
    ...
    active = circuits_data.get(circuit_type, circuits_data["zz_linear"])
    return {
        "status": "success",
        "selected": active["id"],
